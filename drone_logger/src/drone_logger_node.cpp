...
void depthImageCallback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    if(to_log)
    {
        ...
        // to log the image in floating point values (for algo use)
        ...
        //to log the image for viewing purpose
...
int main(int argc, char** argv) {
    ...
    std::string filename;
    nh.param<std::string>("output_dir", output_dir, "/home/lexus/INP2025/marsim_ws/output");
    nh.param<std::string>("filename", filename, "drone_log.txt");
    nh.param<bool>("to_log", to_log, false);
